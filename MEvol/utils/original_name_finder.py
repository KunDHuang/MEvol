...
if __name__ == "__main__":
    ...
    gene_alns = renamingNoutput_nproc(gene_abs_pres_df, postqc_alns, pars['opt_dir'], pars['nproc'])
    gene_alns = [gene_aln for gene_aln in gene_alns if gene_aln is not None]
    gene_alns_2keep = select_gene_alns(gene_alns, coreness = pars['coreness'])
    gene_alns_2rm = list(set(gene_alns) - set(gene_alns_2keep))
    for gene_aln_2rm in gene_alns_2rm:
        subprocess.call("rm {}".format(gene_aln_2rm), shell = True)
